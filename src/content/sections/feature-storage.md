---
enable: true
title: "ZFS Storage Moves With Your Workload"
subtitle: "Storage Vaults"
description: "Copy-on-write clones mean five users sharing a **80GB** model costs **zero extra bytes** and **instantly provision**. Vaults attach to vessels, track usage, and snapshot volumes with a single click."
features:
  - "Zero-cost copy-on-write clones"
  - "Instant provisioning when creating new instances"
  - "One-click snapshots & rollbacks"
image: "../../assets/images/feature-storage.png"
---
